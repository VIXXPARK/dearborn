...
function LikeDislike(props) {
    ...
    const LikeClickHandler = () => {
        ...
        if(Liked){
            axios.post('/api/like/unLikes', {id:props.postId, user : props.userId})
            .then(response => {
                if(response.data.success){
                    setLiked(false)
                }else{
    ...
    const DislikeClickHandler = () => {
        ...
        if(!props.userId) return alert('로그인하세요')
        if(Disliked){
            axios.post('/api/like/unDislikes', {post:props.postId, user : props.userId})
                .then(response => {
                    if(response.data.success){
                        setDisliked(false)
                    }else{
            ...
            if(Liked){
                axios.post('/api/like/unLikes', {post:props.postId, user : props.userId})
                .then(response => {
                    if(response.data.success){
                        setLiked(false)
                    }else{
            ...
            axios.post('/api/like/upDislikes', {post : props.postId, user : props.userId})
            .then(response => {
                if(response.data.success){
                    setDisliked(true)
                    setLikes(Likes-1)
